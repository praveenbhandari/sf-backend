...
def _as_address_rows(addresses: list[AddressCreate]) -> list[Address]:
...
def create_contact(db: Session, payload: ContactCreate) -> Contact:
    ...
    data["addresses"] = _as_address_rows(payload.addresses)
...
def replace_contact(db: Session, contact: Contact, payload: ContactReplace) -> Contact:
    ...
    contact.addresses = _as_address_rows(payload.addresses)
    db.commit()
    db.refresh(contact)
    return contact
...
def update_contact(db: Session, contact: Contact, payload: ContactUpdate) -> Contact:
    ...
    if data.pop("addresses", None) is not None:
        contact.addresses = _as_address_rows(payload.addresses or [])
